...
#    `functionName` is a function's name in your module (e.g., `loopEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Eliot/GitHub/SpaDES.Workshops/loop/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("loop")
  path <- list(modulePath = "C:/Eliot/GitHub/SpaDES.Workshops",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    loop = list(.saveInitialTime = NA)
  )
  ...
  if (exists("loopEvent1", envir = .GlobalEnv)) {
    simOutput <- loopEvent1(mySim)
  } else {
  ...
  if (exists("loopEvent2", envir = .GlobalEnv)) {
    simOutput <- loopEvent2(mySim)
  } else {
