...
Let's define our working directories first
```{r def_dirs, results=FALSE, message=FALSE, warning=TRUE}
## load SpaDES
library(SpaDES)

## check where you0re working first:
getwd()

## SpaDES recursively creates the directory tree inside the current working directory
setPaths(cachePath = "SpaDESintro/cache",
         inputPath = "SpaDESintro/inputs",
         modulePath = "SpaDESintro/modules",
         outputPath = "SpaDESintro/outputs")

## verify that this is want you wanted:
getPaths()
```
  
Now get the modules needed:  
```{r module_downld, message=FALSE, warning=FALSE}
## note how we use the modules folder defined above and chose to download the necessary data
try(downloadModule("LCC2005", path = getPaths()$modulePath, data = TRUE))#, overwrite = TRUE) # overwrite is not yet implemented

## open the description of a module
...
```{r LCC2005_setup, message=FALSE, warning=FALSE}
...
modules <- list("LCC2005") ## selects all modules belonging to LCC2005
...
Briefly explain what the mySim object is  
```{r mysim}
ls.str(mySim)
```
