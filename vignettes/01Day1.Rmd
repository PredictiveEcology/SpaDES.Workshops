...
With `SpaDES` it is possible to change a simulations' parameters and can continue running it.  
What will happen if drought intensity increases?  
...
```{r harvest}
downloadModule(name = "forestHarvest", repo = "CeresBarros/forestHarvest-SpaDESmodule", path = getPaths()$modulePath)
...
## make simulation object
mySim_manag <- simInit(times = times, params = parameters, modules = modules,
                 objects = objects, paths = paths)
...
Are there fewer areas where harvesting can occur if fires become larger (drought is more severe)? 
