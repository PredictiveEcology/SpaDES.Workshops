...
SpaDES.core::downloadModule(name = "integratedSask",
                            path = getPaths()$modulePath, 
                            repo = "tati-micheletti/scfmModules",
                            overwrite = TRUE) # It can't find the repo to download the module, don't know why
...
Steve has a nice set of forest succession and a fire simulation models for a region in Saskatchewan.
```{r fire}
parameters <- list(
...
John, who has a forestry company wants to understand the forest dynamics and the fire to plan how he will harvest in the same region.
...
mySim2 <- simInit(times = times, params = parameters, modules = modulesHarvest, objects = objects, paths = paths, loadOrder = unlist(modules))

# SIMULATION
mySimOut2 <- Cache(spades, mySim2, debug = TRUE)
```
...
mySim3 <- simInit(times = times, params = parameters, modules = modulesHarvest, objects = objects, paths = paths, loadOrder = unlist(modules))
