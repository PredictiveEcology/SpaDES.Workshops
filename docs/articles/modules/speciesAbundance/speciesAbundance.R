...
defineModule(sim, list(
  ...
  version = list(SpaDES.core = "0.1.1", speciesAbundance = "0.0.1"),
...
doEvent.speciesAbundance = function(sim, eventTime, eventType, debug = FALSE) {
  switch(
    ...
    init = {
      ...
      sim <- scheduleEvent(sim, P(sim)$.saveInitialTime, "speciesAbundance", "save")
    },
    plot = {
      # ! ----- EDIT BELOW ----- ! #
      # do stuff for this event
      ...
      # e.g.,
      #sim <- scheduleEvent(sim, time(sim) + P(sim)$.plotInterval, "speciesAbundance", "plot")
    ...
    save = {
      ...
      # e.g.,
      # sim <- scheduleEvent(sim, time(sim) + P(sim)$.saveInterval, "speciesAbundance", "save")
    ...
    event1 = {
      ...
      # e.g.,
      # sim <- scheduleEvent(sim, time(sim) + increment, "speciesAbundance", "templateEvent")
