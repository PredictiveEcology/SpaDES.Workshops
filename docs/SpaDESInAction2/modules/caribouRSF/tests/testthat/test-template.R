...
#    `functionName` is a function's name in your module (e.g., `caribouRSFEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/tmichele/Documents/GitHub/caribouRSF/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("caribouRSF")
  path <- list(modulePath = "/home/tmichele/Documents/GitHub",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    caribouRSF = list(.saveInitialTime = NA)
  )
  ...
  if (exists("caribouRSFEvent1", envir = .GlobalEnv)) {
    simOutput <- caribouRSFEvent1(mySim)
  } else {
  ...
  if (exists("caribouRSFEvent2", envir = .GlobalEnv)) {
    simOutput <- caribouRSFEvent2(mySim)
  } else {
